...
> **เวอร์ชันเอกสาร**: 4.9.1 | **วันอัปเดต**: 2026-07-21 | **สถานะ**: อนุมัติสเปก (Pending Execution Approval)
...
### 16.3 การสร้างคู่มือการใช้งาน User Guide (`documents/manual.md` และ `static/manual.html`) (v4.9.1)

เพื่อรองรับการใช้งานของสัตวแพทย์และบุคลากรทางการแพทย์ในโรงพยาบาลสัตว์ มหาวิทยาลัยเกษตรศาสตร์ ระบบได้รับการเพิ่มคู่มือการใช้งานฉบับสมบูรณ์ทั้งแบบ Markdown (`documents/manual.md`) และแบบหน้าเว็บ HTML ตอบสนองดีไซน์มืด/สว่าง (`static/manual.html`)

1. **การเชื่อมโยงหน้าเว็บ**: ลิงก์ `User Guide` ที่ส่วนท้าย (Footer) ของหน้าหลัก `index.html` เชื่อมโยงเข้าสู่ `manual.html` ให้เปิดอ่านในแท็บใหม่ทันที
2. **เนื้อหาคู่มือประกอบด้วย**:
   - **บทนำและวัตถุประสงค์**: ระบุเป้าหมายเพื่อพัฒนาระบบเวชระเบียนอิเล็กทรอนิกส์ (EHR/EMR) เฉพาะแห่งสำหรับโรงพยาบาลสัตว์ ม.เกษตร
   - **อ้างอิงแหล่งข้อมูลถูกต้อง**: ระบุคำเต็มและที่มาของ SA-PDT และ VSCT จาก *Virginia-Maryland College of Veterinary Medicine (VTSL)*
   - **คู่มือการใช้งาน 5 ขั้นตอน**: การสืบค้นภาษาไทย/อังกฤษ/คำย่อ, การใช้ตัวกรอง, การดูตารางผลลัพธ์, การดูการ์ดมโนทัศน์ และการเพิ่มคำพ้องประจำโครงการ
   - **อภิธานศัพท์และคำย่อการแพทย์**: รวบรวมตัวย่อระบบ (`SA-PDT`, `VSCT`, `SCT-Inter`, `FSN`, `PT`, `EHR/EMR`) และตัวย่อโรค/อาการสัตวแพทย์ที่ใช้บ่อย 14 รายการ (`FLUTD`, `IVDD`, `CCLR`, `CKD`, `CRF`, `AKI`, `GDV`, `UTI`, `TCC`, `SCC`, `MCT`, `OSA`, `HSA`, `KCS`)